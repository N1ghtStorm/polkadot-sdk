...
impl EnvDef {
	pub fn try_from(item: syn::ItemMod) -> syn::Result<Self> {
		...
		let mut syscall_numbers = HashSet::with_capacity(host_funcs.len());
		if !host_funcs
			.iter()
			.all(move |host_fn| syscall_numbers.insert(host_fn.syscall_number))
		{
			return Err(err("Duplicate syscall number. Either you used the same name or there is a hash collision"))
		}
...
fn expand_func_doc(def: &EnvDef) -> TokenStream2 {
	let docs = def.host_funcs.iter().map(|func| {
		...
		let syscall_number = func.syscall_number;
		let func_doc = {
			let func_docs = {
